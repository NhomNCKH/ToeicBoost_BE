...
import { buildShadowingSegments } from '@modules/shadowing/shadowing-segment-builder';
...
@Injectable()
export class DailyDictationService {
  constructor(
    @InjectRepository(DailyDictationContent)
    private readonly contentRepo: Repository<DailyDictationContent>,
    @InjectRepository(DailyDictationSegment)
    private readonly segmentRepo: Repository<DailyDictationSegment>,
  ...
  async adminImportFromYoutube(input: {
    ...
    const content = this.contentRepo.create({
      title: (input.title ?? '').trim() || `Daily Dictation ${youtubeId}`,
      youtubeId,
  ...
  async adminReimportEnglishCaptions(contentId: string) {
    ...
    if (!content) throw new NotFoundException('Không tìm thấy nội dung daily dictation.');
