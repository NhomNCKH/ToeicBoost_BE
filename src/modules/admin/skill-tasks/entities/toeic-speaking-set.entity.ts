...
import { ToeicSpeakingSetItem } from './toeic-speaking-set-item.entity';

@Entity('toeic_speaking_sets')
export class ToeicSpeakingSet extends BaseEntity {
  @Index('uq_toeic_speaking_sets_code', { unique: true })
  ...
  @Index('idx_toeic_speaking_sets_level')
  @Column({
    name: 'level',
    type: 'enum',
    enum: QuestionLevel,
  ...
  @Index('idx_toeic_speaking_sets_status')
  @Column({
    name: 'status',
    type: 'enum',
  ...
  @OneToMany(() => ToeicSpeakingSetItem, (it) => it.set)
  items: ToeicSpeakingSetItem[];
}
